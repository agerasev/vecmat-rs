	...
	($V:ident, $N:expr, $M:expr, $Trait:ident, $method:ident, $op:ident) => (
		impl<T> $Trait<$V<T>> for $V<T> where T: Copy + Default + Num {
			...
			fn $method(self, mat: $V<T>) -> Self::Output {
				mat_map![i, j; $op!(self[(i, j)], mat[(i, j)]); $V, $N, $M]
			}
		}
	)
}
	...
	($V:ident, $N:expr, $M:expr, $Trait:ident, $method:ident, $op:ident) => (
		impl<T> $Trait<T> for $V<T> where T: Copy + Default + Num {
			...
			fn $method(self, a: T) -> Self::Output {
				mat_map![i, j; $op!(self[(i, j)], a); $V, $N, $M]
			}
		}
	)
}
...
macro_rules! mat_all {
	($V:ident, $N:expr, $M:expr) => (
		...
		mat_op_mat_assign!($V, $N, $M, AddAssign, add_assign, op_add);
		mat_op_mat_assign!($V, $N, $M, SubAssign, sub_assign, op_sub);
		mat_op_scal_assign!($V, $N, $M, MulAssign, mul_assign, op_mul);
		mat_op_scal_assign!($V, $N, $M, DivAssign, div_assign, op_div);
	...
	($Vs:ident, $Vr:ident, $N:expr) => (
		impl<T> $Vs<T> where T: Copy + Default {
			pub fn submatrix(self, x: usize, y: usize) -> $Vr<T> {
				mat_map![i, j; self[(i + (i >= x) as usize, j + (j >= y) as usize)]; $Vr, $N-1, $N-1]
			}
		}
...
macro_rules! mat_adj {
	($V:ident, $N:expr) => (
		impl<T> $V<T> where T: Copy + Default + Num + Signed {
			pub fn adj(self) -> $V<T> {
				mat_map![i, j; self.cofactor(j, i); $V, $N, $N]
			}
		}
	)
}
