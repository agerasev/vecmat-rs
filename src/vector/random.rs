macro_rules! vector_random { ($N:expr, $V:ident, $D:ident) => (
    #[derive(Copy, Copy, Debug, PartialEq)]
    ...
    impl<T, D: Distribution<T> + Copy> Distribution<$V<T>> for $D<T, D> {
    ...
    impl<T: Float + Copy> Distribution<$V<T>> for NonZero where StandardNormal: Distribution<$V<T>> {
        fn sample<R: Rng + ?Sized>(&self, rng: &mut R) -> $V<T> {
            loop {
    ...
    impl<T: Float + Copy> Distribution<$V<T>> for Unit where NonZero: Distribution<$V<T>> {
        fn sample<R: Rng + ?Sized>(&self, rng: &mut R) -> $V<T> {
